...
impl DefinitionSource {
    fn label(self) -> &'static str {
        match self {
            Self::ProjectDovai => "Project (.dovai)",
            Self::ProjectClaude => "Project (.claude)",
            Self::UserDovaiHome => "User ($DOVAI_HOME)",
            Self::UserDovai => "User (~/.dovai)",
...
fn discover_definition_roots(cwd: &Path, leaf: &str) -> Vec<(DefinitionSource, PathBuf)> {
    ...
    for ancestor in cwd.ancestors() {
        push_unique_root(
            &mut roots,
            DefinitionSource::ProjectDovai,
            ancestor.join(".dovai").join(leaf),
        );
        push_unique_root(
            &mut roots,
    ...
    if let Ok(dovai_home) = env::var("DOVAI_HOME") {
        push_unique_root(
            &mut roots,
            DefinitionSource::UserDovaiHome,
            PathBuf::from(dovai_home).join(leaf),
        );
    }
    ...
    if let Some(home) = env::var_os("HOME") {
        ...
        push_unique_root(
            &mut roots,
            DefinitionSource::UserDovai,
            home.join(".dovai").join(leaf),
        );
        push_unique_root(
            &mut roots,
...
fn discover_skill_roots(cwd: &Path) -> Vec<SkillRoot> {
    ...
    for ancestor in cwd.ancestors() {
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::ProjectDovai,
            ancestor.join(".dovai").join("skills"),
        ...
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::ProjectDovai,
            ancestor.join(".dovai").join("commands"),
    ...
    if let Ok(dovai_home) = env::var("DOVAI_HOME") {
        let dovai_home = PathBuf::from(dovai_home);
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::UserDovaiHome,
            dovai_home.join("skills"),
            SkillOrigin::SkillsDir,
        );
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::UserDovaiHome,
            dovai_home.join("commands"),
            SkillOrigin::LegacyCommandsDir,
        );
    }
    ...
    if let Some(home) = env::var_os("HOME") {
        ...
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::UserDovai,
            home.join(".dovai").join("skills"),
            SkillOrigin::SkillsDir,
        );
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::UserDovai,
            home.join(".dovai").join("commands"),
            SkillOrigin::LegacyCommandsDir,
        );
        push_unique_skill_root(
            &mut roots,
...
fn default_skill_install_root() -> std::io::Result<PathBuf> {
    if let Ok(dovai_home) = env::var("DOVAI_HOME") {
        return Ok(PathBuf::from(dovai_home).join("skills"));
    }
    if let Some(home) = env::var_os("HOME") {
        return Ok(PathBuf::from(home).join(".dovai").join("skills"));
    }
    Err(std::io::Error::new(
        std::io::ErrorKind::NotFound,
        "unable to resolve a skills install root; set DOVAI_HOME or HOME",
    ))
}
...
fn render_agents_usage(unexpected: Option<&str>) -> String {
    let mut lines = vec![
        ...
        "  Direct CLI       dovai agents".to_string(),
        "  Sources          .dovai/agents, .claude/agents, $DOVAI_HOME/agents".to_string(),
    ];
...
fn render_skills_usage(unexpected: Option<&str>) -> String {
    let mut lines = vec![
        ...
        "  Direct CLI       dovai skills [list|install <path>|help]".to_string(),
        "  Install root     $DOVAI_HOME/skills or ~/.dovai/skills".to_string(),
        "  Sources          .dovai/skills, .claude/skills, legacy /commands".to_string(),
    ];
    if let Some(args) = unexpected {
        lines.push(format!("  Unexpected       {args}"));
    }
...
fn render_mcp_usage(unexpected: Option<&str>) -> String {
    let mut lines = vec![
        ...
        "  Direct CLI       dovai mcp [list|show <server>|help]".to_string(),
        "  Sources          .dovai/settings.json, .dovai/settings.local.json".to_string(),
    ];
    if let Some(args) = unexpected {
        lines.push(format!("  Unexpected       {args}"));
    }
...
mod tests {
    ...
    fn renders_per_command_help_detail() {
        ...
        assert!(help.contains("Summary          Manage Dovai plugins"));
        assert!(help.contains("Aliases          /plugins, /marketplace"));
    ...
    fn lists_agents_from_project_and_user_roots() {
        ...
        let project_agents = workspace.join(".dovai").join("agents");
        let user_home = temp_dir("agents-home");
        let user_agents = user_home.join(".dovai").join("agents");
        ...
        let roots = vec![
            (DefinitionSource::ProjectDovai, project_agents),
            (DefinitionSource::UserDovai, user_agents),
        ];
        let report =
        ...
        assert!(report.contains("Project (.dovai):"));
        assert!(report.contains("planner · Project planner · gpt-5.4 · medium"));
        assert!(report.contains("User (~/.dovai):"));
        assert!(report.contains("(shadowed by Project (.dovai)) planner · User planner"));
    ...
    fn lists_skills_from_project_and_user_roots() {
        ...
        let project_skills = workspace.join(".dovai").join("skills");
        let project_commands = workspace.join(".claude").join("commands");
        let user_home = temp_dir("skills-home");
        let user_skills = user_home.join(".dovai").join("skills");
        ...
        let roots = vec![
            SkillRoot {
                source: DefinitionSource::ProjectDovai,
                path: project_skills,
            ...
            SkillRoot {
                source: DefinitionSource::UserDovai,
                path: user_skills,
                origin: SkillOrigin::SkillsDir,
            },
        ];
        ...
        assert!(report.contains("User (~/.dovai):"));
        assert!(report.contains("(shadowed by Project (.dovai)) plan · User planning guidance"));
    ...
    fn agents_and_skills_usage_support_help_and_unexpected_args() {
        ...
        assert!(agents_help.contains("Direct CLI       dovai agents"));
        ...
        assert!(skills_help.contains("Install root     $DOVAI_HOME/skills or ~/.dovai/skills"));
    ...
    fn mcp_usage_supports_help_and_unexpected_args() {
        ...
        assert!(help.contains("Direct CLI       dovai mcp [list|show <server>|help]"));
    ...
    fn renders_mcp_reports_from_loaded_config() {
        ...
        fs::create_dir_all(workspace.join(".dovai")).expect("workspace config dir");
        fs::create_dir_all(&config_home).expect("config home");
        fs::write(
            workspace.join(".dovai").join("settings.json"),
            r#"{
              "mcpServers": {
                "alpha": {
                  "command": "uvx",
        ...
        fs::write(
            workspace.join(".dovai").join("settings.local.json"),
            r#"{
              "mcpServers": {
                "remote": {
                  "type": "ws",
    ...
    fn installs_skill_into_user_registry_and_preserves_nested_files() {
        ...
        let roots = vec![SkillRoot {
            source: DefinitionSource::UserDovaiHome,
        ...
        assert!(listed.contains("User ($DOVAI_HOME):"));
