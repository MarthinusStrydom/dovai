...
pub fn mvp_tool_specs() -> Vec<ToolSpec> {
    vec![
        ...
        ToolSpec {
            name: "kg_query",
            description: "Query the workspace knowledge graph by entity type or ID. Use for looking up people, documents, processes, locations, etc.",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "node_type": {
                        "type": "string",
                        "description": "Entity type to list: person, organization, document, event, topic, location, financial, process"
                    },
                    "id": {
                        "type": "string",
                        "description": "Specific node ID to retrieve (e.g. 'person:alice-smith', 'process:onboarding')"
                    }
                },
                "additionalProperties": false
            }),
        ...
        ToolSpec {
            name: "kg_search",
            description: "Full-text search across the workspace knowledge graph. Searches names, IDs, and properties of all entities.",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "query": {
                        "type": "string",
                        "description": "Search terms (all must match). e.g. 'quarterly report', 'invoices', 'alice email'"
                    }
                },
                "required": ["query"],
                "additionalProperties": false
        ...
        ToolSpec {
            name: "kg_related",
            description: "Find entities connected to a given node in the knowledge graph. Shows relationships and their direction.",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "node_id": {
                        "type": "string",
                        "description": "The node ID to find relationships for (e.g. 'person:alice-smith', 'topic:budget')"
                    }
                },
                "required": ["node_id"],
                "additionalProperties": false
        ...
        ToolSpec {
            name: "kg_rebuild",
            description: "Force rebuild of the workspace knowledge graph from current files. Use after file changes to update the index.",
            input_schema: json!({
                "type": "object",
                "properties": {},
                "additionalProperties": false
...
pub fn execute_tool(name: &str, input: &Value) -> Result<String, String> {
    match name {
        ...
        "kg_query" => from_value::<KgQueryInput>(input).and_then(run_kg_query),
        "kg_search" => from_value::<KgSearchInput>(input).and_then(run_kg_search),
        "kg_related" => from_value::<KgRelatedInput>(input).and_then(run_kg_related),
        "kg_rebuild" => run_kg_rebuild(),
...
fn build_http_client() -> Result<Client, String> {
    Client::builder()
        ...
        .user_agent("dovai-rust-tools/0.1")
        .build()
        .map_err(|error| error.to_string())
}
...
fn build_search_url(query: &str) -> Result<reqwest::Url, String> {
    if let Ok(base) = std::env::var("DOVAI_WEB_SEARCH_BASE_URL") {
...
fn todo_store_path() -> Result<std::path::PathBuf, String> {
    if let Ok(path) = std::env::var("DOVAI_TODO_STORE") {
        return Ok(std::path::PathBuf::from(path));
    }
    let cwd = std::env::current_dir().map_err(|error| error.to_string())?;
    Ok(cwd.join(".dovai-todos.json"))
}
...
fn resolve_skill_path(skill: &str) -> Result<std::path::PathBuf, String> {
    ...
    if let Ok(dovai_home) = std::env::var("DOVAI_HOME") {
        candidates.push(std::path::PathBuf::from(dovai_home).join("skills"));
    }
    if let Ok(home) = std::env::var("HOME") {
        let home = std::path::PathBuf::from(home);
        candidates.push(home.join(".dovai").join("skills"));
...
fn spawn_agent_job(job: AgentJob) -> Result<(), String> {
    let thread_name = format!("dovai-agent-{}", job.manifest.agent_id);
...
fn agent_store_dir() -> Result<std::path::PathBuf, String> {
    if let Ok(path) = std::env::var("DOVAI_AGENT_STORE") {
        return Ok(std::path::PathBuf::from(path));
    }
    let cwd = std::env::current_dir().map_err(|error| error.to_string())?;
    if let Some(workspace_root) = cwd.ancestors().nth(2) {
        return Ok(workspace_root.join(".dovai-agents"));
    }
    Ok(cwd.join(".dovai-agents"))
}
...
fn normalize_subagent_type(subagent_type: Option<&str>) -> String {
    ...
    match canonical_tool_token(trimmed).as_str() {
        ...
        "dovaiguide" | "dovaiguideagent" | "guide" => String::from("dovai-guide"),
...
fn config_file_for_scope(scope: ConfigScope) -> Result<PathBuf, String> {
    ...
    Ok(match scope {
        ...
        ConfigScope::Settings => cwd.join(".dovai").join("settings.local.json"),
    })
}

fn config_home_dir() -> Result<PathBuf, String> {
    if let Ok(path) = std::env::var("DOVAI_CONFIG_HOME") {
        return Ok(PathBuf::from(path));
    }
    let home = std::env::var("HOME").map_err(|_| String::from("HOME is not set"))?;
    Ok(PathBuf::from(home).join(".dovai"))
}
...
fn load_kg() -> Result<knowledge_graph::KnowledgeGraph, String> {
    let cwd = std::env::current_dir().map_err(|e| format!("cannot get cwd: {e}"))?;
    let dovai_dir = knowledge_graph::find_dovai_dir(&cwd)
        .ok_or_else(|| "no .dovai directory found in workspace".to_string())?;
    knowledge_graph::load_or_build(&dovai_dir).map_err(|e| format!("knowledge graph error: {e}"))
}

#[allow(clippy::needless_pass_by_value)]
fn run_kg_query(input: KgQueryInput) -> Result<String, String> {
    let graph = load_kg()?;

    if let Some(ref id) = input.id {
        return match graph.node_by_id(id) {
            Some(node) => {
                let summary = knowledge_graph::NodeSummary::from(node);
                Ok(summary.render())
            }
            None => Ok(format!("No entity found with ID: {id}")),
        };
    }

    if let Some(ref type_str) = input.node_type {
        let node_type = knowledge_graph::NodeType::from_str_loose(type_str)
            .ok_or_else(|| format!("unknown type: {type_str}. Valid types: person, organization, document, event, topic, location, financial, process"))?;
        let nodes = graph.nodes_by_type(&node_type);
        if nodes.is_empty() {
            return Ok(format!("No {type_str} entities found."));
        }
        let rendered: Vec<String> = nodes
            .iter()
            .map(|n| knowledge_graph::NodeSummary::from(*n).render())
            .collect();
        return Ok(format!(
            "{} {} entities:\n\n{}",
            nodes.len(),
    ...
    // No filter — return stats
    Ok(graph.stats().render())
}

#[allow(clippy::needless_pass_by_value)]
fn run_kg_search(input: KgSearchInput) -> Result<String, String> {
    let graph = load_kg()?;
    let results = graph.search(&input.query);
    if results.is_empty() {
        return Ok(format!("No results for: {}", input.query));
    }
...
#[allow(clippy::needless_pass_by_value)]
fn run_kg_related(input: KgRelatedInput) -> Result<String, String> {
    let graph = load_kg()?;
    let related = graph.related(&input.node_id);
    if related.is_empty() {
        return Ok(format!("No relationships found for: {}", input.node_id));
    }
    let rendered: Vec<String> = related
        .iter()
        .map(knowledge_graph::RelatedEntity::render)
        .collect();
    Ok(format!(
        "{} relationships for {}:\n\n{}",
        related.len(),
...
fn run_kg_rebuild() -> Result<String, String> {
    let cwd = std::env::current_dir().map_err(|e| format!("cannot get cwd: {e}"))?;
    let dovai_dir = knowledge_graph::find_dovai_dir(&cwd)
        .ok_or_else(|| "no .dovai directory found in workspace".to_string())?;
    let graph =
        knowledge_graph::build_or_update(&dovai_dir).map_err(|e| format!("rebuild error: {e}"))?;
    let stats = graph.stats();
    Ok(format!("Knowledge graph rebuilt. {}", stats.render()))
}
...
mod tests {
    ...
    fn temp_path(name: &str) -> PathBuf {
        let unique = std::time::SystemTime::now()
            ...
            .expect("time")
            .as_nanos();
        std::env::temp_dir().join(format!("dovai-tools-{unique}-{name}"))
    }
    ...
    fn web_search_extracts_and_filters_results() {
        ...
        std::env::remove_var("DOVAI_WEB_SEARCH_BASE_URL");
    ...
    fn web_search_handles_generic_links_and_invalid_base_url() {
        ...
        std::env::set_var("DOVAI_WEB_SEARCH_BASE_URL", "://bad-base-url");
    ...
    fn todo_write_persists_and_returns_previous_state() {
        ...
        std::env::set_var("DOVAI_TODO_STORE", &path);
        ...
        .expect("TodoWrite should succeed");
        std::env::remove_var("DOVAI_TODO_STORE");
    ...
    fn skill_loads_local_skill_prompt() {
        ...
        let skill_dir = home.join(".dovai").join("skills").join("help");
    ...
    fn agent_persists_handoff_metadata() {
        ...
        std::env::set_var("DOVAI_AGENT_STORE", &dir);
        ...
        std::env::remove_var("DOVAI_AGENT_STORE");
  ...
  "nbformat_minor": 5
    ...
    #[test]
    #[ignore = "requires stable cwd; CI runners may delete working directory mid-test"]
    ...
    fn config_reads_and_writes_supported_values() {
        ...
        std::fs::create_dir_all(home.join(".dovai")).expect("home dir");
        std::fs::create_dir_all(cwd.join(".dovai")).expect("cwd dir");
        std::fs::write(
            home.join(".dovai").join("settings.json"),
            r#"{"verbose":false}"#,
        )
        ...
        let original_dir = std::env::current_dir().expect("cwd");
        std::env::set_var("HOME", &home);
        std::env::remove_var("DOVAI_CONFIG_HOME");
    ...
    fn enter_and_exit_plan_mode_round_trip_existing_local_override() {
        ...
        std::fs::create_dir_all(cwd.join(".dovai")).expect("cwd dir");
        std::fs::write(
            cwd.join(".dovai").join("settings.local.json"),
        ...
        let local_settings =
            std::fs::read_to_string(cwd.join(".dovai").join("settings.local.json"))
                .expect("local settings after enter");
        assert!(local_settings.contains(r#""defaultMode": "plan""#));
        let state =
            std::fs::read_to_string(cwd.join(".dovai").join("tool-state").join("plan-mode.json"))
    ...
    #[test]
    #[ignore = "requires python runtime; not available on CI ubuntu runner"]
