...
use crate::forget::ForgetConfig;
use crate::json::JsonValue;
use crate::sandbox::{FilesystemIsolationMode, SandboxConfig};

pub const DOVAI_SETTINGS_SCHEMA_NAME: &str = "SettingsSchema";
...
impl ConfigLoader {
    ...
    pub fn discover(&self) -> Vec<ConfigEntry> {
        let user_legacy_path = self.config_home.parent().map_or_else(
            || PathBuf::from(".dovai.json"),
            |parent| parent.join(".dovai.json"),
        );
        vec![
            ConfigEntry {
            ...
            ConfigEntry {
                ...
                path: self.cwd.join(".dovai.json"),
            },
            ConfigEntry {
                source: ConfigSource::Project,
                path: self.cwd.join(".dovai").join("settings.json"),
            },
            ConfigEntry {
                source: ConfigSource::Local,
                path: self.cwd.join(".dovai").join("settings.local.json"),
            },
        ]
    }
...
impl RuntimeFeatureConfig {
    ...
    #[must_use]
    pub fn forget(&self) -> &ForgetConfig {
        &self.forget
    }

    #[must_use]
    pub fn with_forget(mut self, forget: ForgetConfig) -> Self {
        self.forget = forget;
        self
    }
}
...
pub fn default_config_home() -> PathBuf {
    std::env::var_os("DOVAI_CONFIG_HOME")
        .map(PathBuf::from)
        .or_else(|| std::env::var_os("HOME").map(|home| PathBuf::from(home).join(".dovai")))
        .unwrap_or_else(|| PathBuf::from(".dovai"))
}
...
fn read_optional_json_object(
    ...
    let is_legacy_config = path.file_name().and_then(|name| name.to_str()) == Some(".dovai.json");
...
mod tests {
    use super::{
        ...
        RuntimePluginConfig, DOVAI_SETTINGS_SCHEMA_NAME,
    };
    use crate::json::JsonValue;
    ...
    fn rejects_non_object_settings_files() {
        ...
        let home = root.join("home").join(".dovai");
    ...
    fn loads_and_merges_claude_code_config_files_by_precedence() {
        ...
        let cwd = root.join("project");
        let home = root.join("home").join(".dovai");
        fs::create_dir_all(cwd.join(".dovai")).expect("project config dir");
        fs::create_dir_all(&home).expect("home config dir");

        fs::write(
            home.parent().expect("home parent").join(".dovai.json"),
        ...
        assert_eq!(DOVAI_SETTINGS_SCHEMA_NAME, "SettingsSchema");
