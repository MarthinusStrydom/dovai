...
fn discover_instruction_files(cwd: &Path) -> std::io::Result<Vec<ContextFile>> {
    ...
    for dir in directories {
        for candidate in [
            ...
            dir.join(".dovai").join("CLAUDE.md"),
            dir.join(".dovai").join("instructions.md"),
        ] {
...
pub fn load_system_prompt(
    ...
    // Inject knowledge graph summary if a .dovai workspace exists
    if let Some(dovai_dir) = knowledge_graph::find_dovai_dir(&cwd) {
        if let Ok(graph) = knowledge_graph::load_or_build(&dovai_dir) {
            let summary = knowledge_graph::graph_summary(&graph);
            if !summary.is_empty() {
                builder = builder.append_section(summary);
            }
        }
    }
...
fn render_config_section(config: &RuntimeConfig) -> String {
    ...
    if config.loaded_entries().is_empty() {
        lines.extend(prepend_bullets(vec![
            "No Dovai settings files loaded.".to_string()
        ]));
        return lines.join("\n");
    }
...
mod tests {
    ...
    fn discovers_instruction_files_from_ancestor_chain() {
        ...
        fs::create_dir_all(nested.join(".dovai")).expect("nested dovai dir");
        fs::write(root.join("CLAUDE.md"), "root instructions").expect("write root instructions");
        fs::write(root.join("CLAUDE.local.md"), "local instructions")
            .expect("write local instructions");
        fs::create_dir_all(root.join("apps")).expect("apps dir");
        fs::create_dir_all(root.join("apps").join(".dovai")).expect("apps dovai dir");
        fs::write(root.join("apps").join("CLAUDE.md"), "apps instructions")
            .expect("write apps instructions");
        fs::write(
            root.join("apps").join(".dovai").join("instructions.md"),
            "apps dot claude instructions",
        )
        .expect("write apps dot claude instructions");
        fs::write(nested.join(".dovai").join("CLAUDE.md"), "nested rules")
            .expect("write nested rules");
        fs::write(
            nested.join(".dovai").join("instructions.md"),
            "nested instructions",
        )
    ...
    fn displays_context_paths_compactly() {
        assert_eq!(
            display_context_path(Path::new("/tmp/project/.dovai/CLAUDE.md")),
            "CLAUDE.md"
        );
    }
    ...
    fn load_system_prompt_reads_claude_files_and_config() {
        let root = temp_dir();
        fs::create_dir_all(root.join(".dovai")).expect("dovai dir");
        fs::write(root.join("CLAUDE.md"), "Project rules").expect("write instructions");
        fs::write(
            root.join(".dovai").join("settings.json"),
        ...
        let original_dovai_home = std::env::var("DOVAI_CONFIG_HOME").ok();
        std::env::set_var("HOME", &root);
        std::env::set_var("DOVAI_CONFIG_HOME", root.join("missing-home"));
...
",
        ...
        if let Some(value) = original_dovai_home {
            std::env::set_var("DOVAI_CONFIG_HOME", value);
        } else {
    ...
    fn discovers_dot_claude_instructions_markdown() {
        ...
        assert!(context
            .instruction_files
            .iter()
            .any(|file| file.path.ends_with(".dovai/instructions.md")));
        assert!(
