...
use crate::forget::{selective_forget, ForgetConfig, ForgetResult};
    ...
    T: ToolExecutor,
{
    ...
    pub fn new_with_features(
        ...
        Self {
            ...
            forget_config: feature_config.forget().clone(),
    ...
    fn maybe_selective_forget(&mut self) {
        let result = selective_forget(&mut self.session, &self.forget_config);
        if result.blocks_compressed > 0 {
            self.record_selective_forget(&result);
        }
    }

    fn record_selective_forget(&self, result: &ForgetResult) {
        let Some(session_tracer) = &self.session_tracer else {
            return;
        };
        let mut attributes = Map::new();
        attributes.insert(
            "blocks_compressed".to_string(),
            Value::from(result.blocks_compressed as u64),
        );
        attributes.insert(
            "estimated_tokens_saved".to_string(),
            Value::from(result.estimated_tokens_saved as u64),
        );
        session_tracer.record("selective_forget_completed", attributes);
    }
