...
#[test]
#[ignore = "requires release binary: cargo build --release && cargo test -- --ignored"]
...
fn run_case(case: ScenarioCase, workspace: &HarnessWorkspace, base_url: &str) -> ScenarioRun {
    ...
    command
        ...
        .env("DOVAI_CONFIG_HOME", &workspace.config_home)
    ...
    let output = if let Some(stdin) = case.stdin {
        ...
        child.wait_with_output().expect("dovai should finish")
    } else {
...
fn prepare_plugin_fixture(workspace: &HarnessWorkspace) {
    ...
    let manifest_dir = plugin_root.join(".dovai-plugin");
    ...
    fs::write(
        &script_path,
        "#!/bin/sh\nINPUT=$(cat)\nprintf '{\"plugin\":\"%s\",\"tool\":\"%s\",\"input\":%s}\\n' \"$DOVAI_PLUGIN_ID\" \"$DOVAI_TOOL_NAME\" \"$INPUT\"\n",
    )
...
fn unique_temp_dir(label: &str) -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "dovai-mock-parity-{label}-{}-{millis}-{counter}",
        std::process::id()
    ))
}
