...
fn status_command_applies_model_and_permission_mode_flags() {
    ...
    // when
    let output = Command::new(env!("CARGO_BIN_EXE_dovai"))
...
fn slash_command_names_match_known_commands_and_suggest_nearby_unknown_ones() {
    ...
    // when
    let help_output = Command::new(env!("CARGO_BIN_EXE_dovai"))
        .current_dir(&temp_dir)
        .arg("/help")
        .output()
        .expect("dovai should launch");
    let unknown_output = Command::new(env!("CARGO_BIN_EXE_dovai"))
        .current_dir(&temp_dir)
        .arg("/zstats")
        .output()
        .expect("dovai should launch");
...
fn config_command_loads_defaults_from_standard_config_locations() {
    ...
    fs::create_dir_all(temp_dir.join(".dovai")).expect("project config dir should exist");
    ...
    fs::write(temp_dir.join(".dovai.json"), r#"{"model":"sonnet"}"#)
        .expect("write project settings");
    fs::write(
        temp_dir.join(".dovai").join("settings.local.json"),
        r#"{"model":"opus"}"#,
    )
    ...
    let output = command_in(&temp_dir)
        .env("DOVAI_CONFIG_HOME", &config_home)
        .args([
            "--resume",
            session_path.to_str().expect("utf8 path"),
            "/config",
    ...
    assert!(stdout.contains(temp_dir.join(".dovai.json").to_str().expect("utf8 path")));
    assert!(stdout.contains(
        temp_dir
            .join(".dovai")
