...
const DOVAI_MANIFEST_RELATIVE_PATH: &str = ".dovai-plugin/plugin.json";
...
impl PluginTool {
    ...
    pub fn execute(&self, input: &Value) -> Result<String, PluginError> {
        ...
        process
            ...
            .stdout(Stdio::piped())
            .stderr(Stdio::piped())
            .env("DOVAI_PLUGIN_ID", &self.plugin_id)
            .env("DOVAI_PLUGIN_NAME", &self.plugin_name)
            .env("DOVAI_TOOL_NAME", &self.definition.name)
            .env("DOVAI_TOOL_INPUT", &input_json);
        if let Some(root) = &self.root {
            process
                .current_dir(root)
                .env("DOVAI_PLUGIN_ROOT", root.display().to_string());
        }
...
fn plugin_manifest_path(root: &Path) -> Result<PathBuf, PluginError> {
    ...
    let dovai_path = root.join(DOVAI_MANIFEST_RELATIVE_PATH);
    if dovai_path.exists() {
        return Ok(dovai_path);
    }
